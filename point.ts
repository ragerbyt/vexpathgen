let points: HTMLDivElement[] = [], waypoints;
...
function updateDrag(pointElem, newX, newY) {
    ...
    if (index % 5 === 0) {
        ...
        for (let i = -2; i < 3; i++) {
            ...
            if (groupPoint) {
                groupPoint.dataset.x = String(Number(groupPoint.dataset.x) - deltaX);
                groupPoint.dataset.y = String(Number(groupPoint.dataset.y) - deltaY);
            }
        }
        ...
        if (newX && newY) {
            ...
            points[groupStartIndex].dataset.anglex = String((Number(points[index].dataset.x) - Number(points[groupStartIndex].dataset.x)) / Number(pointElem.dataset.dist));
            points[groupStartIndex].dataset.angley = String((Number(points[index].dataset.y) - Number(points[groupStartIndex].dataset.y)) / Number(pointElem.dataset.dist));
        }

 ...
 *
 * @param {ControlPoint[]} points - Array of control points with x and y coordinates.
 * @param {number[]} segments - Array of segment indices (e.g., 0, 1, 2, …).
 */

import { plotData, plotDataaccel } from "./plot";

export function computeBezierWaypoints(points: ControlPoint[], segments: number[]) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    for (let t = 0; t < 1; t += 0.001) {
      ...
      for (let j = 0; j < 6; j++) {
        const pointX = sectpoints[j].x;
        const pointY = sectpoints[j].y;
      ...
      point.velocity = Math.min(
        20,
        point.curvature > 0 ? Math.sqrt(5 / point.curvature) : 20
      );
  ...
  for (let i = 1; i < waypoints.length; i++) {
    ...
    waypoints[i].accel =
      (waypoints[i].velocity ** 2 - waypoints[i + 1].velocity ** 2) / (2 * distance);
  }
...
function computeCurvature(sectpoints: ControlPoint[], t: number): number {
  let vx = 0,
    vy = 0;
  let ax = 0,
    ay = 0;
  ...
  // First derivative
  for (let j = 0; j < 5; j++) {
    const pointX1 = sectpoints[j].x;
    const pointX2 = sectpoints[j + 1].x;
    const pointY1 = sectpoints[j].y;
    const pointY2 = sectpoints[j + 1].y;
  ...
  // Second derivative
  for (let j = 0; j < 4; j++) {
    const pointX0 = sectpoints[j].x;
    const pointX1 = sectpoints[j + 1].x;
    const pointX2 = sectpoints[j + 2].x;
    const pointY0 = sectpoints[j].y;
    const pointY1 = sectpoints[j + 1].y;
    const pointY2 = sectpoints[j + 2].y;
    const coeff2 =
      20 * binomialCoefficient(3, j) * Math.pow(1 - t, 3 - j) * Math.pow(t, j);
